...
def get_links(path:str) -> list:
    with open(path, 'r', encoding='utf-8-sig') as json_file:
        ...
        page_range = range(int(product_raw_json['start_page']), int(product_raw_json['end_page'])+1)
        links = []
        for page in page_range:
            links.extend(product_raw_json[str(page)])
...
def test():
    ...
    product_links = get_links("./api_call/20240326_17h34m_extra_battery_product_link.json")
